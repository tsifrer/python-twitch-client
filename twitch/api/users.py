...
class Users(TwitchAPI):
    @oauth_required
    async def get(self):
        response = await self._request_get("user")
        return User.construct_from(response)

    async def get_by_id(self, user_id):
        response = await self._request_get("users/{}".format(user_id))
        return User.construct_from(response)

    @oauth_required
    async def get_emotes(self, user_id):
        response = await self._request_get("users/{}/emotes".format(user_id))
        return response["emoticon_sets"]

    @oauth_required
    async def check_subscribed_to_channel(self, user_id, channel_id):
    ...
    async def check_follows_channel(self, user_id, channel_id):
        response = await self._request_get(
    ...
    @oauth_required
    async def follow_channel(self, user_id, channel_id, notifications=False):
    ...
    @oauth_required
    async def unfollow_channel(self, user_id, channel_id):
        await self._request_delete("users/{}/follows/channels/{}".format(user_id, channel_id))

    @oauth_required
    async def get_user_block_list(self, user_id, limit=25, offset=0):
        if limit > 100:
            raise TwitchAttributeException(
                "Maximum number of objects returned in one request is 100"
            )
        ...
        response = await self._request_get("users/{}/blocks".format(user_id), params=params)
    ...
    async def unblock_user(self, user_id, blocked_user_id):
        await self._request_delete("users/{}/blocks/{}".format(user_id, blocked_user_id))

    async def translate_usernames_to_ids(self, usernames):
        if isinstance(usernames, list):
            usernames = ",".join(usernames)

        response = await self._request_get("users?login={}".format(usernames))
